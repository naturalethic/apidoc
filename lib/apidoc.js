...
var app = {
	...
	parsers: {
		...
		apidefaultgroup          : "./plugins/parser_api_default_group.js",
	...
	try
	{
		...
		for(var i = 0; i < files.length; i += 1)
		{
			...
			if(parsedFile)
			{
				...
				{
					if (item.global.group) defaultGroup = item.global.group;
					if (item.local.type)
					{
						if (!item.local.name)  item.local.name  = 'anon' + item.index;
						if (!item.local.group) item.local.group = defaultGroup;
					}
				});
			}
		} // for
