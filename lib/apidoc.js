		...
		for(var i = 0; i < files.length; i += 1)
		{
			...
			if(parsedFile)
			{
				...
				parsedFile.forEach(function(item)
				{
					if (item.local.type)
					{
						if (!item.local.name)  item.local.name  = item.local.title.replace(/\s+/g, '-').replace(/[\[\]]/, '').toLowerCase();
						if (!item.local.group) item.local.group = path.basename(filename).replace(new RegExp(path.extname(filename) + '$'), '');
					}
				});
			}
		} // for
